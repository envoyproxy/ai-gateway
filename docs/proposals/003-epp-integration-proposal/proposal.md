...
But it cannot support rule level InferencePool support. EnvoyExtensionPolicy can only target on HTTPRoute/Gateway Level, so it cannot support multiple InferencePool or mixed use AIServiceBackend in one AIGatewayRoute.
