...
description: Token counting APIs, per-request backend credentials, stream idle timeout with failover, MCP hostname routing and backend selection, OpenTelemetry GenAI tracing, and HTTP CONNECT egress
...
Upgrading from **v1.0** requires no CRD migrations. New fields are optional and backward compatible. Review the Helm security-context change below if you override the controller image.
