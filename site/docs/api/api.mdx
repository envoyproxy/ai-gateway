...
/><ApiField
  ...
  required="false"
  description="OIDCExchangeToken specifies the oidc configurations used to obtain an oidc token. The oidc token will be<br />used to obtain temporary credentials to access Azure.<br />In the case both OIDC and clientSecret is defined, OIDC exchange tokens will be used to obtain Azure access token."
/>
