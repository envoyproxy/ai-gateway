...
- [BackendSecurityPolicyGeminiAPIKey](#backendsecuritypolicygeminiapikey)
...
/><ApiField
  ...
  type="enum"
  required="false"
  description="APISchemaGemini is the schema for Gemini models accessed via generativelanguage.googleapis.com.<br />Uses API key authentication (injected as query parameter ?key=).<br />Translation logic is identical to GCPVertexAI but targets the Gemini API endpoint.<br />https://ai.google.dev/gemini-api/docs/api-overview<br />"
/><ApiField
  name="GCPAnthropic"
  type="enum"
  required="false"
...
#### BackendSecurityPolicyGeminiAPIKey
...
<ApiField
  ...
  type="[SecretObjectReference](https://gateway-api.sigs.k8s.io/references/spec/#gateway.networking.k8s.io/v1.SecretObjectReference)"
  required="true"
  description="SecretRef is the reference to the secret containing the Gemini API key.<br />ai-gateway must be given the permission to read this secret.<br />The key of the secret should be `apiKey`."
/>
...
/><ApiField
  name="geminiAPIKey"
  type="[BackendSecurityPolicyGeminiAPIKey](#backendsecuritypolicygeminiapikey)"
  required="false"
  description="GeminiAPIKey is a mechanism to access Gemini backend(s). The API key will be injected as a query parameter.<br />https://ai.google.dev/gemini-api/docs/api-key"
/>
