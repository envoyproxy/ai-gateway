...
Here's a complete example that demonstrates InferencePool integration with AI Gateway:

### 1. Deploy the Complete Stack

First, deploy all the necessary components:

```bash
# Install Gateway API Inference Extension CRDs
kubectl apply -f https://github.com/kubernetes-sigs/gateway-api-inference-extension/releases/latest/download/manifests.yaml

# Deploy sample vLLM deployment
kubectl apply -f https://github.com/kubernetes-sigs/gateway-api-inference-extension/raw/main/config/manifests/vllm/cpu-deployment.yaml
...
### 2. Create AI Gateway Configuration

Create your AI Gateway configuration that uses the installed InferencePool:
...
spec:
  ...
  - matches:
    ...
    - group: inference.networking.x-k8s.io
      kind: InferencePool
      name: vllm-llama3-8b-instruct  # This references the InferencePool created by the installation
```

### 3. Test the Configuration

Once deployed, you can test the configuration:
...
curl -X POST http://<gateway-address>/v1/chat/completions \
