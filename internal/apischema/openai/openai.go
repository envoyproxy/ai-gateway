...
func (r *ResponseInputItemUnionParam) UnmarshalJSON(data []byte) error {
	...
	case "tool_search_output":
		var tso ResponseToolSearchOutputItem
...
// A call to search the tools available to the model.
//
// The properties Arguments and Type are required for input items. IDs, execution,
// status, and creator metadata are populated when the item is returned by the API.
...
// The tools loaded by a tool search call.
//
// The properties Tools and Type are required for input items. IDs, execution,
// status, and creator metadata are populated when the item is returned by the API.
...
func (r *ResponseOutputItemUnion) UnmarshalJSON(data []byte) error {
	...
	case "tool_search_output":
		var t ResponseToolSearchOutputItem
		if err := json.Unmarshal(data, &t); err != nil {
			return err
		}
