...
type ChatCompletionResponseChoiceMessage struct {
	...
	// ExtraFields is used to hold any non-standard fields from the backend,
	// like "reasoning_content". It is not marshaled directly.
	ExtraFields map[string]interface{} `json:"-"`
}

// MarshalJSON implements a custom marshaler for ChatCompletionResponseChoiceMessage.
// It merges the standard fields with the contents of ExtraFields.
func (c ChatCompletionResponseChoiceMessage) MarshalJSON() ([]byte, error) {
	// Create a map to build the final JSON object.
	m := make(map[string]interface{})

	// Manually add the standard fields from the struct to the map
	...
	// Iterate through the ExtraFields map and add its contents to the top level.
	for k, v := range c.ExtraFields {
		m[k] = v
	}

	// Marshal the final, merged map into a JSON byte slice.
