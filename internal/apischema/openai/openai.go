...
type ChatCompletionResponseChoiceMessage struct {
	...
	Annotations AnnotationArray `json:"annotations"`
...
// AnnotationArray is a custom type for handling arrays of annotations in JSON.
//
// The annotation array is not nullable, so even if the array is empty or nil, the JSON
// representation will be an empty array `[]`.
type AnnotationArray []string

var emptyArrayBytes = []byte("[]")

// MarshalJSON implements json.Marshaler for AnnotationArray.
func (a AnnotationArray) MarshalJSON() ([]byte, error) {
	if len(a) == 0 { // Handle both nil and empty slice cases.
		return emptyArrayBytes, nil
	}
	return json.Marshal([]string(a))
}

// UnmarshalJSON implements json.Unmarshaler for AnnotationArray.
func (a *AnnotationArray) UnmarshalJSON(data []byte) error {
	var temp []string
	if err := json.Unmarshal(data, &temp); err != nil {
		*a = []string{}
	} else {
		*a = temp // Convert the temporary slice to AnnotationArray.
	}
	return nil
}
...
type CompletionTokensDetails struct {
	// Text input tokens present in the prompt. TODO: no idea why this is undocumented on the official OpenAI API docs.
