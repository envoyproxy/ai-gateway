...
func mcpConfig(mcpRoutes []aigv1a1.MCPRoute) *filterapi.MCPConfig {
	...
	for i := range mcpRoutes {
		...
		for j := range route.Spec.BackendRefs {
			backendRef := route.Spec.BackendRefs[j]
			mcpBackend := filterapi.MCPBackend{
				// MCPRoute doesn't support cross-namespace backend reference so just use the name.
				Name: filterapi.MCPBackendName(backendRef.Name),
				Path: ptr.Deref(backendRef.Path, "/mcp"),
			}
			if backendRef.ToolSelector != nil {
				mcpBackend.ToolSelector = &filterapi.MCPToolSelector{
					Include:      backendRef.ToolSelector.Include,
					IncludeRegex: backendRef.ToolSelector.IncludeRegex,
				}
			}
