...
func (c *BackendSecurityPolicyController) reconcile(ctx context.Context, bsp *aigv1a1.BackendSecurityPolicy) (res ctrl.Result, err error) {
	...
	if bsp.Spec.Type != aigv1a1.BackendSecurityPolicyTypeAPIKey {
		...
		secretName, useGeneratedSecret := getBSPGeneratedSecretName(bsp)
		if useGeneratedSecret {
			var secret *corev1.Secret
			secret, err = rotators.LookupSecret(ctx, c.client, bsp.Namespace, secretName)
			if err != nil {
				panic(fmt.Errorf("BUG: failed to lookup backend security policy secret %s/%s: %w", bsp.Namespace, bsp.Name, err))
			}
...
// if there's any. This returns false if not.
func getBSPGeneratedSecretName(bsp *aigv1a1.BackendSecurityPolicy) (string, bool) {
	switch bsp.Spec.Type {
	case aigv1a1.BackendSecurityPolicyTypeAWSCredentials:
	...
	case aigv1a1.BackendSecurityPolicyTypeAPIKey:
		return "", false // APIKey does not require rotation.
	default:
