...
func (c *BackendSecurityPolicyController) rotateCredential(ctx context.Context, bsp *aigv1b1.BackendSecurityPolicy) (res ctrl.Result, err error) {
	...
	case aigv1b1.BackendSecurityPolicyTypeAWSCredentials:
		...
		case bsp.Spec.AWSCredentials.CredentialsFile != nil && bsp.Spec.AWSCredentials.CredentialsFile.SecretRef != nil:
			// Read the credentials secret and check if role_arn is present.
			...
			var secretRoleArn string
			secretRoleArn, err = c.getRoleARNFromSecret(ctx, secretNamespace, secretName)
			if err != nil {
				return ctrl.Result{}, err
			}
			if secretRoleArn == "" {
				// No role_arn in credentials file, use credentials directly without rotation.
				c.logger.Info("Using AWS credentials file directly without role assumption, skipping rotation",
					"namespace", bsp.Namespace, "name", bsp.Name)
				return ctrl.Result{}, nil
			}
			region := bsp.Spec.AWSCredentials.Region
			rotator, err = rotators.NewAWSAssumeRoleRotator(ctx, c.client, nil, c.kube, c.logger, bsp.Namespace, bsp.Name, preRotationWindow, secretRoleArn, region, secretName, secretNamespace)
			if err != nil {
				return ctrl.Result{}, err
			}
		default:
...
// getRoleARNFromSecret reads a credentials secret and extracts the role_arn
// from the AWS credentials file content. Returns empty string if role_arn is not present.
func (c *BackendSecurityPolicyController) getRoleARNFromSecret(ctx context.Context, namespace, secretName string) (string, error) {
	secret, err := rotators.LookupSecret(ctx, c.client, namespace, secretName)
	if err != nil {
		return "", fmt.Errorf("failed to lookup credentials secret %s/%s: %w", namespace, secretName, err)
	}
	credData, exists := secret.Data[rotators.AwsCredentialsKey]
	if !exists {
		return "", nil
	}
	parsed, err := rotators.ParseAWSCredentialsFile(string(credData))
	if err != nil {
		return "", nil // If parsing fails, treat as no role_arn.
	}
	return parsed.RoleARN, nil
}
...
func getBSPGeneratedSecretName(bsp *aigv1b1.BackendSecurityPolicy) string {
	switch bsp.Spec.Type {
	case aigv1b1.BackendSecurityPolicyTypeAWSCredentials:
		if bsp.Spec.AWSCredentials.OIDCExchangeToken == nil && bsp.Spec.AWSCredentials.CredentialsFile == nil {
			return ""
		}
	case aigv1b1.BackendSecurityPolicyTypeAzureCredentials:
