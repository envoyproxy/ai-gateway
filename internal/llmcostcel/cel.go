...
func NewProgram(expr string) (prog cel.Program, err error) {
	...
	_, err = EvaluateProgram(prog, "dummy", "dummy", "dummy", 0, 0, 0, 0, 0)
	if err != nil {
...
func EvaluateProgram(prog cel.Program, modelName, backend, routeName string, inputTokens, cachedInputTokens, cacheCreationInputTokens, outputTokens, totalTokens uint32) (uint64, error) {
