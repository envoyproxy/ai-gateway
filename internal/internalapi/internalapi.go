...
const (
	...
	// LuaFilterOrderAnnotation is the annotation key on an EnvoyExtensionPolicy that
	// controls where its Lua filters are placed relative to the AI Gateway ext_proc filter.
	LuaFilterOrderAnnotation = "aigateway.envoyproxy.io/lua-filter-order"

	// LuaFilterOrderBeforeExtProc is the value for LuaFilterOrderAnnotation that causes
	// Lua filters to be placed before the AI Gateway ext_proc filter.
	LuaFilterOrderBeforeExtProc = "before-extproc"
)
