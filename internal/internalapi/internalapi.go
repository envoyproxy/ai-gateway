...
const (
	...
	// InternalMetadataMergedBackendNamesKey stores a route rule's MergeBackends cluster to backend
	// mapping, encoded by EncodeMergedBackendNames. A merged cluster is shared, so it cannot carry
	// a rule-scoped backend name itself; the route does, keyed by the serving cluster's name.
	InternalMetadataMergedBackendNamesKey = "merged_backend_names"
...
const (
	...
	// XDSRouteMetadataMergedBackendNamesPath is the full attribute path to the route rule's
	// MergeBackends cluster to backend mapping.
	XDSRouteMetadataMergedBackendNamesPath = "xds.route_metadata.filter_metadata['aigateway.envoy.io']['merged_backend_names']"
	// XDSClusterNamePath is the attribute holding the serving cluster's name, which selects the
	// entry to use from XDSRouteMetadataMergedBackendNamesPath.
	XDSClusterNamePath = "xds.cluster_name"
...
const (
	// mergedBackendPairSeparator separates the entries of an encoded MergeBackends mapping.
	mergedBackendPairSeparator = ";"
	// mergedBackendKVSeparator separates a cluster name from its backend name.
	mergedBackendKVSeparator = "="
)

// EncodeMergedBackendNames encodes a cluster to backend mapping as "<cluster>=<backend>;...",
// sorted so the same rule always produces the same string and Envoy does not drain the route on
// every translation.
//
// It is a flat string rather than a nested struct because Envoy does not serialize struct-valued
// metadata into ext_proc attributes: the external processor receives the literal "CelMap value".
// Neither separator can occur in a Kubernetes-derived cluster or backend name, so no escaping is
// needed; an entry containing one is dropped rather than corrupting its neighbours.
func EncodeMergedBackendNames(mapping map[string]string) string {
	clusters := make([]string, 0, len(mapping))
	for cluster, backend := range mapping {
		if strings.ContainsAny(cluster, mergedBackendPairSeparator+mergedBackendKVSeparator) ||
			strings.ContainsAny(backend, mergedBackendPairSeparator+mergedBackendKVSeparator) {
			continue
		}
	...
	for i, cluster := range clusters {
		if i > 0 {
			sb.WriteString(mergedBackendPairSeparator)
		}
		sb.WriteString(cluster)
		sb.WriteString(mergedBackendKVSeparator)
		sb.WriteString(mapping[cluster])
	}
	return sb.String()
}

// LookupMergedBackendName returns the backend name that an EncodeMergedBackendNames mapping
// associates with clusterName.
func LookupMergedBackendName(encoded, clusterName string) (string, bool) {
	if encoded == "" || clusterName == "" {
		return "", false
	}
	for rest := encoded; rest != ""; {
		var pair string
		pair, rest, _ = strings.Cut(rest, mergedBackendPairSeparator)
		cluster, backend, ok := strings.Cut(pair, mergedBackendKVSeparator)
		if ok && cluster == clusterName && backend != "" {
			return backend, true
		}
	}
	return "", false
