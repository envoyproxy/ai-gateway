...
type mockAudioTranscriptionMetrics struct {
...
func (m *mockAudioTranscriptionMetrics) StartRequest(_ map[string]string) { m.requestStart = time.Now() }
func (m *mockAudioTranscriptionMetrics) SetOriginalModel(originalModel string) {
	m.originalModel = originalModel
}
func (m *mockAudioTranscriptionMetrics) SetRequestModel(requestModel string) {
	m.requestModel = requestModel
}
func (m *mockAudioTranscriptionMetrics) SetResponseModel(responseModel string) {
	m.responseModel = responseModel
}
func (m *mockAudioTranscriptionMetrics) SetBackend(backend *filterapi.Backend) { m.backend = backend.Name }
func (m *mockAudioTranscriptionMetrics) RecordTokenUsage(_ context.Context, inputTokens uint32, _ map[string]string) {
	m.tokenUsageCount += int(inputTokens)
}
func (m *mockAudioTranscriptionMetrics) RecordRequestCompletion(_ context.Context, success bool, _ map[string]string) {
	if success {
...
var _ metrics.AudioTranscriptionMetrics = &mockAudioTranscriptionMetrics{}
...
func (m *mockAudioTranscriptionTranslator) ResponseBody(_ map[string]string, body io.Reader, _ bool) (*extprocv3.HeaderMutation, *extprocv3.BodyMutation, translator.LLMTokenUsage, internalapi.ResponseModel, error) {
	if m.expResponseBody != nil {
		buf, err := io.ReadAll(body)
		require.NoError(m.t, err)
		require.Equal(m.t, m.expResponseBody, buf)
	}
	return m.retHeaderMutation, m.retBodyMutation, m.retUsedToken, internalapi.ResponseModel(m.retResponseModel), m.retErr
}
...
func TestAudioTranscriptionProcessorFactory(t *testing.T) {
	t.Run("router filter", func(t *testing.T) {
		factory := AudioTranscriptionProcessorFactory(func() metrics.AudioTranscriptionMetrics {
			return &mockAudioTranscriptionMetrics{}
		})
		processor, err := factory(&processorConfig{}, map[string]string{}, slog.Default(), tracing.NoopTracing{}, false)
	...
	t.Run("upstream filter", func(t *testing.T) {
		...
		processor, err := factory(&processorConfig{}, map[string]string{}, slog.Default(), tracing.NoopTracing{}, true)
		require.NoError(t, err)
		require.NotNil(t, processor)
...
func TestAudioTranscriptionProcessorUpstreamFilter_SelectTranslator(t *testing.T) {
	...
	t.Run("gemini", func(t *testing.T) {
		p := &audioTranscriptionProcessorUpstreamFilter{}
		err := p.selectTranslator(filterapi.VersionedAPISchema{Name: filterapi.APISchemaGemini})
		require.NoError(t, err)
		require.NotNil(t, p.translator)
	})
...
func TestAudioTranscriptionProcessorUpstreamFilter_ProcessRequestHeaders(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		p := &audioTranscriptionProcessorUpstreamFilter{
			...
			metrics:                &mockAudioTranscriptionMetrics{},
		}
...
func TestAudioTranscriptionProcessorUpstreamFilter_ProcessResponseBody(t *testing.T) {
	...
	t.Run("token accumulation", func(t *testing.T) {
		mockTranslator := &mockAudioTranscriptionTranslator{
			t:                 t,
			retHeaderMutation: &extprocv3.HeaderMutation{},
			retBodyMutation:   &extprocv3.BodyMutation{},
		...
		require.NoError(t, err)
		require.Equal(t, uint32(100), p.costs.InputTokens)
		require.Equal(t, uint32(100), p.costs.TotalTokens)
		...
		require.NoError(t, err)
		require.Equal(t, uint32(200), p.costs.InputTokens)
		require.Equal(t, uint32(200), p.costs.TotalTokens)
	})
}
...
func TestAudioTranscriptionProcessorUpstreamFilter_SetBackend(t *testing.T) {
	...
	t.Run("gemini backend with api key", func(t *testing.T) {
		routeProcessor := &audioTranscriptionProcessorRouterFilter{
			originalRequestBody:    &openai.AudioTranscriptionRequest{Model: "whisper-1"},
			originalRequestBodyRaw: []byte("test"),
		}
		...
		backend := &filterapi.Backend{
			Name:   "gemini-backend",
			Schema: filterapi.VersionedAPISchema{Name: filterapi.APISchemaGemini},
			Auth: &filterapi.BackendAuth{
				GeminiAPIKey: &filterapi.GeminiAPIKeyAuth{},
			},
		}
...
func TestParseAudioTranscriptionBody(t *testing.T) {
	...
	t.Run("multipart with all fields", func(t *testing.T) {
		...
		writer.WriteField("temperature", "0.5")
	...
	t.Run("multipart with temperature as invalid json", func(t *testing.T) {
		...
		writer.WriteField("model", "whisper-1")
		writer.WriteField("temperature", "not-a-number")
