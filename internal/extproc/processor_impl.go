...
func (u *upstreamProcessor[ReqT, RespT, RespChunkT, EndpointSpecT]) ProcessRequestHeaders(ctx context.Context, _ *corev3.HeaderMap) (res *extprocv3.ProcessingResponse, err error) {
	...
	// x-ai-eg-aws-signing-host is an internal, controller-derived value consumed by the AWS backend
	// auth handler (via the upstream-host metadata attribute). Strip any copy — including one a
