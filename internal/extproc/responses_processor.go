...
// SetUpstreamFilter sets the upstream filter for this processor.
func (r *responsesProcessorRouterFilter) SetUpstreamFilter(filter Processor) {
	r.upstreamFilter = filter
	r.upstreamFilterCount++
}
...
func (u *responsesProcessorUpstreamFilter) ProcessResponseHeaders(ctx context.Context, headers *corev3.HeaderMap) (res *extprocv3.ProcessingResponse, err error) {
	defer func() {
		if err != nil {
			u.metrics.RecordRequestCompletion(ctx, false, u.requestHeaders)
			u.metrics.RecordError(ctx, u.originalRequestBody.Model, u.backendName, "response_headers_error")
		}
	}()
...
func (u *responsesProcessorUpstreamFilter) ProcessResponseBody(ctx context.Context, body *extprocv3.HttpBody) (res *extprocv3.ProcessingResponse, err error) {
	...
	defer func() {
		if err != nil || recordRequestCompletionErr {
			u.metrics.RecordError(ctx, u.originalRequestBody.Model, u.backendName, "response_body_error")
			u.metrics.RecordRequestCompletion(ctx, false, u.requestHeaders)
			return
		}
		if body.EndOfStream {
	...
	u.costs.InputTokens += tokenUsage.InputTokens
	u.costs.OutputTokens += tokenUsage.OutputTokens
	u.costs.TotalTokens += tokenUsage.TotalTokens
	...
	if body.EndOfStream {
		u.metrics.RecordTokenUsage(ctx, u.costs.InputTokens, u.costs.OutputTokens, u.requestHeaders)

		// End the tracing span on successful completion
		if u.span != nil {
			u.span.EndSpan()
		}
	}
