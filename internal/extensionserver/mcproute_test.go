...
func TestServer_modifyMCPGatewayGeneratedCluster(t *testing.T) {
	const (
		...
		mergedProxyCluster = "backend/default/default-myroute-mcp-proxy/0"
	)
	tests := []struct {
		name             string
		routes           []*routev3.RouteConfiguration
		...
		},
		{
			...
			routes: []*routev3.RouteConfiguration{mcpMainProxyRouteConfig(mergedProxyCluster)},
			clusters: []*clusterv3.Cluster{
				edsCluster("backend/default/some-other-backend/0"),
			},
			expectedClusters: []*clusterv3.Cluster{
				edsCluster("backend/default/some-other-backend/0"),
			},
...
// ("backend/<ns>/<name>/<port>") with no inline endpoints. The proxy cluster, identified via
...
func TestPostTranslateModify_MCPProxyMergeBackends(t *testing.T) {
	...
	const mergedProxyCluster = "backend/default/default-myroute-mcp-proxy/0"
