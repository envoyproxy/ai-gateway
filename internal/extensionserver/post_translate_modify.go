...
func (s *Server) insertRouterLevelAIGatewayExtProc(listener *listenerv3.Listener, routeConfig *routev3.RouteConfiguration) {
	const routerLevelExtProcName = "envoy.filters.http.ext_proc/aigateway"
	...
	for _, vh := range routeConfig.VirtualHosts {
		...
		resourcesLoop:
			...
			if aiGatewayGenerated {
				...
				route.TypedPerFilterConfig[routerLevelExtProcName] = mustToAny(&routev3.FilterConfig{
					Config: &anypb.Any{},
				})
			}
		}
	...
	if relevantListener {
		...
		for _, currChain := range filterChains {
			...
			for _, f := range httpConManager.HttpFilters {
				if f.Name == routerLevelExtProcName {
					// Nothing to do, the filter is already there.
					return
				}
			}
