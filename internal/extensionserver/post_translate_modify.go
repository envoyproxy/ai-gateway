...
func (s *Server) maybeModifyCluster(cluster *clusterv3.Cluster) error {
	...
	extProcConfig.MutationRules = &mutation_rulesv3.HeaderMutationRules{
...
func (s *Server) enableRouterLevelAIGatewayExtProcOnRoute(routeConfig *routev3.RouteConfiguration) (bool, error) {
	enabled := false
	// Use ExtProcOverrides to enable the filter at the route level.
	// Per Envoy docs: "A set of overrides in a more specific configuration
	// will override a 'disabled' flag set in a less-specific one."
	// IMPORTANT: An empty ExtProcOverrides does NOT enable a disabled filter.
	// At least one field must be set in the overrides to supersede the listener-level
	// disabled state. We set ProcessingMode to match the listener-level config.
	fcAny, err := toAny(&extprocv3.ExtProcPerRoute{
	...
	if err != nil {
		return false, fmt.Errorf("failed to marshal ExtProcPerRoute to Any: %w", err)
	}
	...
	for _, vh := range routeConfig.VirtualHosts {
		for _, route := range vh.Routes {
			...
			if aiGatewayGenerated {
				s.log.Info("enabling AI Gateway extproc filter for route", "route", route.Name, "virtualHost", vh.Name)
				enabled = true
				if route.TypedPerFilterConfig == nil {
					route.TypedPerFilterConfig = make(map[string]*anypb.Any)
				}
	...
	if enabled {
		s.log.Info("enabled AI Gateway extproc filter for AIGateway routes", "routeConfig", routeConfig.Name)
	}
	return enabled, nil
}
...
func (s *Server) insertRouterLevelAIGatewayExtProc(listener *listenerv3.Listener) error {
	...
	for _, currChain := range filterChains {
		...
		extProcFilter := &httpconnectionmanagerv3.HttpFilter{
			Name:       aiGatewayExtProcName,
			Disabled:   false, // Enable by default; ext_proc must run BEFORE routing to set the model header used for route matching.
