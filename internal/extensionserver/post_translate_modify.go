...
const (
	...
	noBackendRefIndex     = -1
	catchAllRouteNamePrefix = "aigateway-shared-route-not-found"
...
func (s *Server) PostTranslateModify(ctx context.Context, req *egextension.PostTranslateModifyRequest) (*egextension.PostTranslateModifyResponse, error) {
	...
	// Inject shared catch-all routes before route-level extproc enablement.
	// Ordering matters: maybeModifyListenerAndRoutes enables extproc per-route by walking
	// the current route list. If catch-all routes are injected later, they won't get
	// per-route extproc config and can bypass model-aware routing behavior.
	s.maybeInjectSharedCatchAllRoutes(req.Routes)
	// Modify listeners and routes to support InferencePool backends and route-level extproc.
...
// enableRouterLevelAIGatewayExtProcOnRoute enables the AI Gateway extproc filter on routes that
// participate in model-aware handling:
//   - normal AIGateway-generated forwarding routes, and
//   - synthetic shared catch-all routes injected by maybeInjectSharedCatchAllRoutes.
//
// Shared catch-all routes are xDS-only and do not carry the original AIGateway annotation, so we
// match them by stable name prefix. Returns true if at least one route was marked.
func (s *Server) enableRouterLevelAIGatewayExtProcOnRoute(routeConfig *routev3.RouteConfiguration) (bool, error) {
	...
	for _, vh := range routeConfig.VirtualHosts {
		for _, route := range vh.Routes {
			aiGatewayGenerated := s.isRouteGeneratedByAIGateway(route) || isSharedCatchAllRouteName(route.Name)
			if aiGatewayGenerated {
				...
				internalMetadata := ensureRouteInternalMetadata(route)
				// Preserve an existing route-name mapping when already set (for example on
				// synthesized shared catch-all routes) so we don't overwrite origin context.
				if _, ok := internalMetadata.Fields[internalapi.InternalMetadataRouteNameKey]; !ok {
					...
					internalMetadata.Fields[internalapi.InternalMetadataRouteNameKey] = structpb.NewStringValue(routeName)
				}
			}
		}
	}
...
func isSharedCatchAllRouteName(name string) bool {
	return name == catchAllRouteNamePrefix || strings.HasPrefix(name, catchAllRouteNamePrefix+"/")
}
...
// when there is at least one forwarding AIGateway-generated route in that virtual host.
//
// Notes:
//   - Existing synthetic catch-all routes are removed first, then re-synthesized (idempotent update).
//   - We intentionally ignore direct-response-only AIGateway routes when deciding to inject.
//   - The synthesized route carries an internal route-name mapping copied from one source route so
//     downstream extproc stages can keep stable route context.
//   - The route exists only in xDS and disappears automatically when no source routes remain.
func (s *Server) maybeInjectSharedCatchAllRoutes(routeConfigs []*routev3.RouteConfiguration) {
	for _, routeConfig := range routeConfigs {
		for _, vh := range routeConfig.VirtualHosts {
			...
			for _, route := range vh.Routes {
				if isSharedCatchAllRouteName(route.Name) {
					continue
				}
				withoutCatchAll = append(withoutCatchAll, route)
				// Only inject shared catch-all when the vhost has at least one forwarding
				// AIGateway route. This avoids catch-all injection based solely on non-forwarding
				// routes (e.g. direct responses) that should not drive normal traffic routing.
				if route.GetRoute() != nil && s.isRouteGeneratedByAIGateway(route) {
					hasAIGatewayRoute = true
					if sourceRouteName == "" {
						sourceRouteName = routeNameFromEnvoyGatewayMetadata(route)
						if sourceRouteName == "" {
							sourceRouteName = routeNameFromRouteConfigName(routeConfig.Name)
						}
						if sourceRouteName == "" {
			...
			catchAllRoute := &routev3.Route{
				Name: fmt.Sprintf("%s/%s", catchAllRouteNamePrefix, vh.Name),
			...
			if sourceRouteName != "" {
				ensureRouteInternalMetadata(catchAllRoute).Fields[internalapi.InternalMetadataRouteNameKey] = structpb.NewStringValue(sourceRouteName)
			}
			vh.Routes = append(vh.Routes, catchAllRoute)
		}
	}
}
