...
// that are generated by AIGateway. It modifies the route configuration to enable the AI Gateway
// extproc filter for those routes. It returns true if any route was modified.
	...
	for _, vh := range routeConfig.VirtualHosts {
		for _, route := range vh.Routes {
			...
			if aiGatewayGenerated {
				...
				// Enable the AI Gateway extproc filter for this route.
...
func (s *Server) insertRouterLevelAIGatewayExtProc(listener *listenerv3.Listener) error {
	...
	for _, currChain := range filterChains {
		...
		if err != nil {
			return fmt.Errorf("failed to marshal AI Gateway ExternalProcessor to Any: %w", err)
		}
