...
func TestNew(t *testing.T) {
	s, err := New(newFakeClient(), logr.Discard(), udsPath, false, nil, nil, "envoy-ai-gateway-ratelimit.envoy-gateway-system")
	require.NoError(t, err)
	require.NotNil(t, s)
}
...
func Test_maybeModifyCluster(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run("error/"+tc.errLog, func(t *testing.T) {
			...
			s, err := New(c, logr.FromSlogHandler(slog.NewTextHandler(&buf, &slog.HandlerOptions{})), udsPath, false, nil, nil, "envoy-ai-gateway-ratelimit.envoy-gateway-system")
			require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			s, err := New(c, logr.FromSlogHandler(handler), udsPath, false, nil, nil, "envoy-ai-gateway-ratelimit.envoy-gateway-system")
			require.NoError(t, err)
...
func TestPatchListenerWithInferencePoolFilters(t *testing.T) {
	...
	t.Run("listener with filter chains but no HCM", func(t *testing.T) {
		var buf bytes.Buffer
		server, err := New(newFakeClient(), logr.FromSlogHandler(slog.NewTextHandler(&buf, &slog.HandlerOptions{})), udsPath, false, nil, nil, "envoy-ai-gateway-ratelimit.envoy-gateway-system")
		require.NoError(t, err)
...
func TestPostClusterModify(t *testing.T) {
	logger := logr.Discard()
	s, err := New(newFakeClient(), logger, udsPath, false, nil, nil, "envoy-ai-gateway-ratelimit.envoy-gateway-system")
	require.NoError(t, err)
...
func TestPostTranslateModify(t *testing.T) {
	...
	t.Run("with log header mapping inserts header_to_metadata filter", func(t *testing.T) {
		s, err := New(newFakeClient(), logger, udsPath, false, nil, ptr.To("agent-session-id:session.id"), "envoy-ai-gateway-ratelimit.envoy-gateway-system")
		require.NoError(t, err)
		hcm := &httpconnectionmanagerv3.HttpConnectionManager{
	...
	t.Run("without log header mapping leaves filters untouched", func(t *testing.T) {
		s, err := New(newFakeClient(), logger, udsPath, false, nil, ptr.To(""), "envoy-ai-gateway-ratelimit.envoy-gateway-system")
		require.NoError(t, err)
		hcm := &httpconnectionmanagerv3.HttpConnectionManager{
			HttpFilters: []*httpconnectionmanagerv3.HttpFilter{{Name: wellknown.Router}},
		}
