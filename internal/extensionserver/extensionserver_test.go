...
func TestNew(t *testing.T) {
	s, err := New(newFakeClient(), logr.Discard(), udsPath, false, nil, nil, nil)
	require.NoError(t, err)
	require.NotNil(t, s)
}
...
func TestServerPostTranslateModify(t *testing.T) {
	...
	t.Run("only UDS cluster exists", func(t *testing.T) {
		s, err := New(newFakeClient(), logr.Discard(), udsPath, false, nil, nil, nil)
		require.NoError(t, err)
		req := &egextension.PostTranslateModifyRequest{Clusters: []*clusterv3.Cluster{
			{Name: extProcUDSClusterName},
		}}
		res, err := s.PostTranslateModify(t.Context(), req)
		require.NoError(t, err)
		require.Len(t, res.Clusters, 1) // Only the existing UDS cluster
		require.Equal(t, extProcUDSClusterName, res.Clusters[0].Name)
	})
}
...
func Test_maybeModifyCluster(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run("error/"+tc.errLog, func(t *testing.T) {
			...
			s, err := New(c, logr.FromSlogHandler(slog.NewTextHandler(&buf, &slog.HandlerOptions{})), udsPath, false, nil, nil, nil)
			require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			s, err := New(c, logr.FromSlogHandler(handler), udsPath, false, nil, nil, nil)
			require.NoError(t, err)
...
func TestMaybeModifyClusterExtended(t *testing.T) {
	...
	t.Run("cluster with existing HttpProtocolOptions", func(t *testing.T) {
		s, err := New(c, logr.Discard(), udsPath, false, nil, nil, nil)
		require.NoError(t, err)
...
func TestPatchListenerWithInferencePoolFilters(t *testing.T) {
	...
	t.Run("listener with filter chains but no HCM", func(t *testing.T) {
		var buf bytes.Buffer
		server, err := New(newFakeClient(), logr.FromSlogHandler(slog.NewTextHandler(&buf, &slog.HandlerOptions{})), udsPath, false, nil, nil, nil)
		require.NoError(t, err)
...
func TestPostClusterModify(t *testing.T) {
	logger := logr.Discard()
	s, err := New(newFakeClient(), logger, udsPath, false, nil, nil, nil)
	require.NoError(t, err)
...
func TestPostTranslateModify(t *testing.T) {
	...
	t.Run("with log header mapping inserts header_to_metadata filter", func(t *testing.T) {
		s, err := New(newFakeClient(), logger, udsPath, false, nil, ptr.To("agent-session-id:session.id"), nil)
		require.NoError(t, err)
		hcm := &httpconnectionmanagerv3.HttpConnectionManager{
	...
	t.Run("without log header mapping leaves filters untouched", func(t *testing.T) {
		s, err := New(newFakeClient(), logger, udsPath, false, nil, ptr.To(""), nil)
		require.NoError(t, err)
		hcm := &httpconnectionmanagerv3.HttpConnectionManager{
			HttpFilters: []*httpconnectionmanagerv3.HttpFilter{{Name: wellknown.Router}},
		}
