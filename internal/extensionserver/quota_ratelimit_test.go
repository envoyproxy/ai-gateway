...
func TestEnableQuotaRateLimitOnRoute_WithBucketRules(t *testing.T) {
	t.Run("exact header match generates HeaderValueMatch action", func(t *testing.T) {
		...
		expectedKey := translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 0, 0)
	...
	t.Run("distinct header generates RequestHeaders action", func(t *testing.T) {
		...
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 0, 0), rh.DescriptorKey)
	})
	...
	t.Run("multiple headers across selectors are flattened", func(t *testing.T) {
		...
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 0, 0), hvm0.DescriptorKey)
		require.Equal(t, "x-api-key", hvm0.Headers[0].Name)

		hvm1 := ruleEntry.Actions[3].GetHeaderValueMatch()
		require.NotNil(t, hvm1)
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 0, 1), hvm1.DescriptorKey)
	...
	t.Run("multiple bucket rules for same model", func(t *testing.T) {
		...
		require.NotNil(t, hvm1)
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 1, 0), hvm1.DescriptorKey)
		...
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 0, 0), sdHvm0.DescriptorKey)
		...
		require.Equal(t, translator.BucketRuleDescriptorKey("test-backend", "gpt-4", 1, 0), sdHvm1.DescriptorKey)
...
func TestBuildClientSelectorActions(t *testing.T) {
	...
	t.Run("single header exact match", func(t *testing.T) {
		...
		require.NotNil(t, hvm)
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "gpt-4", 0, 0), hvm.DescriptorKey)
	})
	...
	t.Run("single header distinct match", func(t *testing.T) {
		...
		require.NotNil(t, rh)
		require.Equal(t, "x-user-id", rh.HeaderName)
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "gpt-4", 0, 0), rh.DescriptorKey)
	})
	...
	t.Run("multiple headers across selectors flattened", func(t *testing.T) {
		...
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "model", 1, 0), actions[0].GetHeaderValueMatch().DescriptorKey)

		// h2: HeaderValueMatch
		require.NotNil(t, actions[1].GetHeaderValueMatch())
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "model", 1, 1), actions[1].GetHeaderValueMatch().DescriptorKey)

		// h3: RequestHeaders (Distinct)
		require.NotNil(t, actions[2].GetRequestHeaders())
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "model", 1, 2), actions[2].GetRequestHeaders().DescriptorKey)
	})
}
...
func TestBuildHeaderMatchAction(t *testing.T) {
	t.Run("exact match creates HeaderValueMatch with ExpectMatch true", func(t *testing.T) {
		...
		require.NotNil(t, hvm)
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "gpt-4", 0, 0), hvm.DescriptorKey)
		require.Equal(t, translator.BucketRuleDescriptorKey("my-backend", "gpt-4", 0, 0), hvm.DescriptorValue)
		require.True(t, hvm.ExpectMatch.Value)
		require.Len(t, hvm.Headers, 1)
		require.Equal(t, "x-api-key", hvm.Headers[0].Name)
		require.Equal(t, "premium", hvm.Headers[0].GetStringMatch().GetExact())
	...
	t.Run("descriptor key encodes model, rule, and match index", func(t *testing.T) {
		...
		hvm := action.GetHeaderValueMatch()
		require.Equal(t, "rule-my-backend-claude-3-match-2", hvm.DescriptorKey)
		require.Equal(t, "rule-my-backend-claude-3-match-2", hvm.DescriptorValue)
	})
}
