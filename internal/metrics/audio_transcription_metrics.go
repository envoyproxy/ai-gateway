...
type AudioTranscriptionMetricsFactory func() AudioTranscriptionMetrics

type AudioTranscriptionMetrics interface {
...
func NewAudioTranscriptionFactory(meter metric.Meter, requestHeaderAttributeMapping map[string]string) AudioTranscriptionMetricsFactory {
	...
	return func() AudioTranscriptionMetrics {
		return &audioTranscription{
			baseMetrics: b.newBaseMetrics(genaiOperationAudioTranscription),
		}
	}
}

func (a *audioTranscription) RecordTokenUsage(ctx context.Context, inputTokens uint32, requestHeaders map[string]string) {
