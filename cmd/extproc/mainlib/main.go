...
func parseAndValidateFlags(args []string) (extProcFlags, error) {
	...
	fs.StringVar(&flags.endpointPrefixes,
		"endpointPrefixes",
		"",
		"Comma-separated key-value pairs for endpoint prefixes. Format: openaiPrefix:/v1,coherePrefix:/cohere/v2,anthropicPrefix:/anthropic/v1.",
	)
...
func Main(ctx context.Context, args []string, stderr io.Writer) (err error) {
	...
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAIPrefix, "/chat/completions"), extproc.ChatCompletionProcessorFactory(chatCompletionMetrics))
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAIPrefix, "/completions"), extproc.CompletionsProcessorFactory(completionMetrics))
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAIPrefix, "/embeddings"), extproc.EmbeddingsProcessorFactory(embeddingsMetrics))
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAIPrefix, "/images/generations"), extproc.ImageGenerationProcessorFactory(imageGenerationMetrics))
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.CoherePrefix, "/rerank"), extproc.RerankProcessorFactory(rerankMetrics))
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAIPrefix, "/models"), extproc.NewModelsProcessor)
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.AnthropicPrefix, "/messages"), extproc.MessagesProcessorFactory(messagesMetrics))
