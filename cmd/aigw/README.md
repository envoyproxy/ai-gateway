...
1. **Start the services**:
   ...
   ```bash
   COMPOSE_PROFILES= compose < profile > docker -f docker-compose-otel.yaml up --build --wait -d
   ```
...
2. **Send test requests**:
   ...
   ```bash
   COMPOSE_PROFILES= compose < profile > docker -f docker-compose-otel.yaml run --build --rm chat-completion
   COMPOSE_PROFILES= compose < profile > docker -f docker-compose-otel.yaml run --build --rm create-embeddings
   COMPOSE_PROFILES= compose < profile > docker -f docker-compose-otel.yaml run --build --rm mcp
   ```
