...
helm-test: helm-package  ## Test the helm chart with a dummy version.
	...
	@$(GO_TOOL) helm template ${HELM_CHART_PATH} --show-only templates/admission_webhook.yaml | grep -q "^kind: Secret$$"
	@! $(GO_TOOL) helm template ${HELM_CHART_PATH} --show-only templates/admission_webhook.yaml --set controller.mutatingWebhook.createSecret=false | grep -q "^kind: Secret$$"
