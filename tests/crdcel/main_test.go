...
func TestBackendSecurityPolicies(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			expErr: "spec.azureCredentials.applicationIdentity.clientID in body should be at least 1 chars long",
		},
		{
			name:   "azure_credentials_missing_tenant_id.yaml",
			expErr: "spec.azureCredentials.applicationIdentity.tenantID in body should be at least 1 chars long",
		},
