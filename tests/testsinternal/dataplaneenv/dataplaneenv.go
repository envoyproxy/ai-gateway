...
func init() {
	var err error
	extprocBin, err = testsinternal.BuildGoBinaryOnDemand("EXTPROC_BIN", "extproc", "./cmd/extproc")
	if err != nil {
...
type TestEnvironment struct {
	...
	upstreamOut, extprocOut, envoyStdout, envoyStderr testsinternal.OutBuffer
...
func StartTestEnvironment(t testing.TB,
	...
	ports := testsinternal.RequireRandomPorts(t, defaultPortCount+len(miscPortDefauls))
	...
	var buffers []testsinternal.OutBuffer
	if okToDumpLogOnFailure {
		buffers = testsinternal.DumpLogsOnFail(t, labels...)
	} else {
		buffers = testsinternal.CaptureOutput(labels...)
	}
...
func requireEnvoy(t testing.TB,
	...
	cmd.Dir = testsinternal.FindProjectRoot()
