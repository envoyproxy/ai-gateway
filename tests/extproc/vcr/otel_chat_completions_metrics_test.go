...
func TestOtelOpenAIChatCompletions_metrics(t *testing.T) {
	...
	for _, tc := range buildOtelChatMetricsTestCases() {
		...
		t.Run(tc.cassette.String(), func(t *testing.T) {
			...
			if tc.isStreaming && !tc.isError {
				verifyTimeToFirstTokenMetricsWithOriginal(t, metrics, originalModel, requestModel, responseModel)
...
// verifyTimeToFirstTokenMetricsWithOriginal verifies the gen_ai.server.time_to_first_token metric including original model attribute.
func verifyTimeToFirstTokenMetricsWithOriginal(t *testing.T, metrics *metricsv1.ScopeMetrics, originalModel, requestModel, responseModel string) {
	t.Helper()
