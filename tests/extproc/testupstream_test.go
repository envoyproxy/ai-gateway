...
func testWithTestUpstream(t *testing.T, dynamicModules bool) {
	...
	for _, tc := range []struct {
		...
		//		},
		{
			...
			path:            "/anthropic/v1/messages",
			method:          http.MethodPost,
			requestBody:     `{"model":"anthropic.claude-2-sonnet-20240229-v1:0","max_tokens":100,"messages":[{"role":"user","content":[{"type":"text","text":"Hello from AWS!"}]}],"stream":false}`,
			...
			expPath:         "/model/anthropic.claude-2-sonnet-20240229-v1:0/invoke",
			responseStatus:  strconv.Itoa(http.StatusOK),
