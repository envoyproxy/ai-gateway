...
func TestWithTestUpstream(t *testing.T) {
	...
	config := &filterapi.Config{
		...
		Backends: []filterapi.Backend{
			...
			testUpstreamHuggingFaceTEIBackend,
	...
	for _, tc := range []struct {
		...
		{
			name:            "huggingfacetei - /cohere/v2/rerank",
			backend:         "huggingfacetei",
			path:            "/cohere/v2/rerank",
			method:          http.MethodPost,
			requestBody:     `{"model":"BAAI/bge-reranker-v2-m3","query":"What is machine learning?","documents":["ML is a subset of AI.","The weather is sunny."]}`,
			expPath:         "/rerank",
			expRequestBody:  `{"query":"What is machine learning?","texts":["ML is a subset of AI.","The weather is sunny."]}`,
			responseBody:    `[{"index":0,"score":0.999},{"index":1,"score":0.00002}]`,
			expStatus:       http.StatusOK,
			expResponseBody: `{"results":[{"index":0,"relevance_score":0.999},{"index":1,"relevance_score":0.00002}]}`,
		},
		{
			name:            "huggingfacetei - /cohere/v2/rerank - top_n truncates results",
			backend:         "huggingfacetei",
			path:            "/cohere/v2/rerank",
			method:          http.MethodPost,
			requestBody:     `{"model":"BAAI/bge-reranker-v2-m3","query":"What is machine learning?","documents":["ML is a subset of AI.","The weather is sunny.","AI includes ML."],"top_n":1}`,
			expPath:         "/rerank",
			expRequestBody:  `{"query":"What is machine learning?","texts":["ML is a subset of AI.","The weather is sunny.","AI includes ML."]}`,
			responseBody:    `[{"index":2,"score":0.9},{"index":0,"score":0.5},{"index":1,"score":0.1}]`,
			expStatus:       http.StatusOK,
			expResponseBody: `{"results":[{"index":2,"relevance_score":0.9}]}`,
		},
		{
			name:            "huggingfacetei - /cohere/v2/rerank - TEI error mapped to Cohere",
			backend:         "huggingfacetei",
			path:            "/cohere/v2/rerank",
			method:          http.MethodPost,
			requestBody:     `{"model":"BAAI/bge-reranker-v2-m3","query":"q","documents":["d"]}`,
			expPath:         "/rerank",
			responseStatus:  strconv.Itoa(http.StatusRequestEntityTooLarge),
			responseBody:    `{"error":"batch size 1025 > maximum allowed batch size 1024","error_type":"Validation"}`,
			expStatus:       http.StatusRequestEntityTooLarge,
			expResponseBody: `{"message":"batch size 1025 > maximum allowed batch size 1024"}`,
		},
		{
