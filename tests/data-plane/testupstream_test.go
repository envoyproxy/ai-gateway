...
func TestWithTestUpstream(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:            "gcp-anthropicai - /v1/chat/completions - missing max_tokens uses default",
			backend:         "gcp-anthropicai",
			path:            "/v1/chat/completions",
			method:          http.MethodPost,
			requestBody:     `{"model":"claude-3-sonnet","messages":[{"role":"user","content":"Hello"}]}`,
			expRequestBody:  `{"max_tokens":4096,"messages":[{"content":[{"text":"Hello","type":"text"}],"role":"user"}],"anthropic_version":"vertex-2023-10-16"}`,
