...
func TestWithTestUpstream(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:              "gcp-vertexai - /v1/images/generations - gemini basic",
			backend:           "gcp-vertexai",
			path:              "/v1/images/generations",
			method:            http.MethodPost,
			requestBody:       `{"model":"gemini-2.5-flash-image","prompt":"a serene mountain landscape at sunrise","size":"1024x1024","n":1}`,
			expRequestBody:    `{"contents":[{"parts":[{"text":"a serene mountain landscape at sunrise"}],"role":"user"}],"tools":null,"generation_config":{"candidateCount":1}}`,
			...
			responseBody:      `{"candidates":[{"content":{"parts":[{"inlineData":{"mimeType":"image/png","data":"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="}}],"role":"model"},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":10,"candidatesTokenCount":0,"totalTokenCount":10},"modelVersion":"gemini-2.5-flash-image-001"}`,
			expStatus:         http.StatusOK,
			expResponseBody:   `{"created":-62135596800,"data":[{"b64_json":"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="}],"usage":{"total_tokens":10,"input_tokens":10,"output_tokens":0}}`,
		},
		{
			name:              "gcp-vertexai - /v1/images/generations - gemini uri response",
			backend:           "gcp-vertexai",
			path:              "/v1/images/generations",
			method:            http.MethodPost,
			requestBody:       `{"model":"gemini-2.5-flash-image","prompt":"abstract digital art","n":2}`,
			expRequestBody:    `{"contents":[{"parts":[{"text":"abstract digital art"}],"role":"user"}],"tools":null,"generation_config":{"candidateCount":2}}`,
			expPath:           "/v1/projects/gcp-project-name/locations/gcp-region/publishers/google/models/gemini-2.5-flash-image:generateContent",
			expRequestHeaders: map[string]string{"Authorization": "Bearer " + fakeGCPAuthToken},
			responseStatus:    strconv.Itoa(http.StatusOK),
			responseBody:      `{"candidates":[{"content":{"parts":[{"fileData":{"fileUri":"gs://bucket/image1.png","mimeType":"image/png"}}],"role":"model"}},{"content":{"parts":[{"fileData":{"fileUri":"gs://bucket/image2.png","mimeType":"image/png"}}],"role":"model"}}],"usageMetadata":{"promptTokenCount":8,"totalTokenCount":8}}`,
			expStatus:         http.StatusOK,
			expResponseBody:   `{"created":-62135596800,"data":[{"url":"gs://bucket/image1.png"},{"url":"gs://bucket/image2.png"}],"usage":{"total_tokens":8,"input_tokens":8,"output_tokens":0}}`,
		},
		{
			...
			path:              "/v1/images/generations",
			method:            http.MethodPost,
			requestBody:       `{"model":"imagen-4.0-generate-001","prompt":"futuristic cityscape at night"}`,
			expRequestBody:    `{"contents":[{"parts":[{"text":"futuristic cityscape at night"}],"role":"user"}],"tools":null,"generation_config":{}}`,
			expPath:           "/v1/projects/gcp-project-name/locations/gcp-region/publishers/google/models/imagen-4.0-generate-001:generateContent",
			expRequestHeaders: map[string]string{"Authorization": "Bearer " + fakeGCPAuthToken},
			responseStatus:    strconv.Itoa(http.StatusOK),
			responseBody:      `{"candidates":[{"content":{"parts":[{"inlineData":{"mimeType":"image/png","data":"aGVsbG93b3JsZA=="}}]}}],"usageMetadata":{"promptTokenCount":12,"totalTokenCount":12}}`,
			expStatus:         http.StatusOK,
			expResponseBody:   `{"created":-62135596800,"data":[{"b64_json":"aGVsbG93b3JsZA=="}],"usage":{"total_tokens":12,"input_tokens":12,"output_tokens":0}}`,
		},
		{
			name:            "gcp-vertexai - /v1/images/generations - error response",
			backend:         "gcp-vertexai",
			path:            "/v1/images/generations",
			method:          http.MethodPost,
			requestBody:     `{"model":"gemini-2.5-flash-image","prompt":""}`,
			expPath:         "/v1/projects/gcp-project-name/locations/gcp-region/publishers/google/models/gemini-2.5-flash-image:generateContent",
			responseStatus:  strconv.Itoa(http.StatusBadRequest),
			responseBody:    `{"error":{"code":400,"message":"Prompt cannot be empty","status":"INVALID_ARGUMENT"}}`,
			expStatus:       http.StatusBadRequest,
			expResponseBody: `{"type":"error","error":{"type":"INVALID_ARGUMENT","code":"400","message":"Prompt cannot be empty"}}`,
		},
		{
			name:            "unknown path",
			path:            "/unknown",
